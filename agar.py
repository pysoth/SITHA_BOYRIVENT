...
class Camera:
    ...
    def centre(self,blobOrPos):
        ...
        """
        if isinstance(blobOrPos, avatar):
...
class CellList(Drawable):
    ...
    def __init__(self, surface, camera, numOfCells):
        ...
        self.list = []
        for i in range(self.count): self.list.append(creep(self.surface, self.camera))
...
creep = CellList(MAIN_SURFACE, cam, 2000)
blob = avatar (MAIN_SURFACE, cam, "SITHA_ADRIEN")
...
while(True):
    ...
    blob.move()
    blob.collisionDetection(creep.list)
    cam.update(blob)
